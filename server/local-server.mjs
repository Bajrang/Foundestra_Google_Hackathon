...
const PREFIX = '/make-server-f7922768';
...
app.get(`${PREFIX}/health`, (c) => c.json({ status: 'ok', mode: 'local' }));
...
app.get(`${PREFIX}/vertex-config`, (c) => {
  ...
  const hasServiceAccount = !!sa || !!process.env.GOOGLE_SERVICE_ACCOUNT_KEY;
  ...
  return c.json({
    ...
    configuration: {
      ...
      recommendation: authMethod === 'none'
        ? 'Set GOOGLE_APPLICATION_CREDENTIALS or VERTEX_AI_API_KEY in .env for live AI suggestions.'
...
serve({ fetch: app.fetch, port: PORT }, (info) => {
  const sa = getServiceAccountConfig();
  console.log(`Local API server running at http://localhost:${info.port}${PREFIX}`);
  console.log(sa ? `✓ Vertex AI: service account configured (model: ${process.env.GOOGLE_GENAI_VERTEXAI_MODEL || 'gemini-2.5-flash'})` : '○ Vertex AI: not configured');
